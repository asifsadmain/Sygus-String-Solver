...
class ProgramList:
    ...
    """
    Returns a tuple of the next cheapest cost-tuple in the form of the arities containing the cheapest tuple. 
    The tuple is of the form: [list_arity, costs]
    """
...
class BeeSearch:
    ...
    def search(self, bound, string_literals_list, integer_literals_list,
        ...
        self.plist.init_plist(string_literals_list, integer_literals_list, boolean_literals, string_variables_list, integer_variables_list)

        while(True):
            lst, cost = self.plist.get_next_cheapest()

            if (cost > bound):
                break

            for p in self.grow(lst, cost):
